...
OUT = ROOT / "data"
CANDIDATES = {
    "Canada": ["XIU.TO","XIC.TO","VCN.TO","ZCN.TO","VFV.TO","ZSP.TO","XQQ.TO","QQC.F.TO","XEF.TO","VIU.TO","XEC.TO","VEE.TO","XBB.TO","ZAG.TO","VAB.TO","XRE.TO","XEI.TO","VDY.TO","XDIV.TO","CDZ.TO","TEC.TO","HCLN.TO","HURA.TO","CEF.TO","ZLB.TO","ZLU.TO","XMV.TO","XUU.TO","XAW.TO","ZSP.U.TO"],
    "India": ["NIFTYBEES.NS","JUNIORBEES.NS","BANKBEES.NS","ITBEES.NS","PHARMABEES.NS","CPSEETF.NS","GOLDBEES.NS","SILVERBEES.NS","MON100.NS","SETFNIF50.NS","ICICIB22.NS","HDFCNIFTY.NS","NIFTYIETF.NS","LOWVOL1.NS","MID150BEES.NS","MIDCAPETF.NS","ALPHA.NS","MOM100.NS","AUTOBEES.NS","CONSUMBEES.NS","FMCGIETF.NS","HEALTHIETF.NS","METALIETF.NS","PSUBNKBEES.NS","PVTBANIETF.NS","ITETF.NS","GILT5YBEES.NS","LIQUIDBEES.NS","MNC.NS"]
}
def num(v):
    try:
    ...
    try:
        ...
        for frame in (getattr(fund,"top_holdings",None),getattr(fund,"equity_holdings",None)):
            ...
            for symbol,row in frame.iterrows():
                ...
                result.append({"symbol":symbol,"weight":weight})
            if result: return result[:25]
    except Exception as exc: print(f"Holdings unavailable for {ticker}: {exc}")
    return []
    ...
    for filename in ("canada.json","india.json"):
        try:
            ...
            for stock in data.get("stocks",[]):
                ...
                if ticker and stock.get("name"): names[ticker]=stock["name"]
        except Exception: pass
    return names
def enrich_holding_names(items,names):
    return [{"ticker":names.get(str(x.get("symbol") or "").upper(),x.get("symbol")),"symbol":x.get("symbol"),"weight":x.get("weight")} for x in items]
...
def main():
    ...
    for market,filename in (("Canada","etf_canada.json"),("India","etf_india.json")):
        ...
        (OUT/filename).write_text(json.dumps({"market":market,"updatedAt":datetime.now(timezone.utc).isoformat(),"count":len(rows),"etfs":rows},indent=2,ensure_ascii=False),encoding="utf-8")
