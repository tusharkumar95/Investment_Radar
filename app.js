...
function normalizeMarketData(data) {
    ...
    data.stocks = data.stocks.map(stock => {
        const priceData = stock.price && typeof stock.price === "object" ? stock.price : {};
        const priceValue = Number.isFinite(Number(stock.price)) ? Number(stock.price) : Number.isFinite(Number(priceData.current)) ? Number(priceData.current) : Number.isFinite(Number(stock.currentPrice)) ? Number(stock.currentPrice) : null;
        ...
        ownership.insiderHolding = ownership.insiderHolding ?? ownership.insiderOwnership ?? null;
        ownership.institutionalHolding = ownership.institutionalHolding ?? ownership.institutionalOwnership ?? null;
        ownership.promoterHolding = ownership.promoterHolding ?? ownership.promoterOwnership ?? null;
        ...
        if (valuation.peg == null && Number(valuation.pe) > 0 && Number(fundamentals.epsGrowth5Y) > 0) valuation.peg = Number(valuation.pe) / Number(fundamentals.epsGrowth5Y);
        if (valuation.priceToFcf == null && Number(valuation.marketCap) > 0 && Number(fundamentals.freeCashFlow) > 0) valuation.priceToFcf = Number(valuation.marketCap) / Number(fundamentals.freeCashFlow);
        return { ...stock, name: stock.name || stock.shortName || stock.longName || stock.companyName || stock.symbol || stock.ticker || "Unknown", ticker: stock.ticker || stock.symbol || stock.id || "", type: stock.type || stock.quoteType || "Investment", price: priceValue, currentPrice: priceValue, priceData, fundamentals, valuation, ownership, dividend, technical };
    });
    return data;
}
...
function formatPlain(value) { const n = numeric(value); return n === null ? "—" : n.toLocaleString("en-US", { maximumFractionDigits: 2 }); }
